...
> [DefaultDispatcher-worker-1] 토큰 업데이트 시작
[DefaultDispatcher-worker-3] 네트워크 요청
...
- ***의문!***
    ...
    - ***join이 있는지 없는지 어떻게 알고 일시 중단을 시키는 것일까?***
    - "어떻게`join()`이 있는지 없는지를 알아서**일시중단(suspend)**시키는 걸까?"
    - **코루틴 런타임이 어떻게 그걸 알아차리는가?**
    - 정답
        - `join()`은`suspend`함수이기 때문에, 코루틴이 실행될 때 런타임이 그 지점에서 “일시중단해야 한다”는 걸 이미 알고 있음.
        - 즉, 코루틴 빌더(`launch`)는 단순히 Job을 반환하고, 그**Job에 대해`join()`을 호출하면**, 내부적으로**suspension point**가 등록되어**현재 코루틴의 진행을
          멈춥니다.**
        - `join()`내부에서는`suspendCoroutine { continuation -> ... }`같은 방식으로**현재 코루틴의 실행을 멈추고**Job이 끝나면 다시`resume()`시켜줍니다.
        ...
        - `suspendCancellableCoroutine`을 써서 **중단 지점(suspension point)**을 만들고 Job이 끝나면 resume되도록 되어 있어요.
...
- 실제 개발 시에는 **서로 독립적인 여러 코루틴을 병렬로 실행한 후 실행한 요청들이 모두 끝날 때까지 기다렸다가 다음 작업을 진행하는 것이 효율적**이다. 예를 들어, SNS 앱의 이미지 업로드 기능을 개발한다고
  해보자. 이 경우 사용자가 복수의 이미지를 선택했다면, 이미지를 모두 변환한 후 업로드 작업을 진행하도록 만들어야 한다. 이 때 이미지가 3개라면, 코루틴을 하나만 만들어 한 번에 이미지를 하나씩 변환하기보다
  코루틴을 3개 만들어 각 이미지 변환 작업을 병렬로 실행한 후 결과를 취합해 업로드 작업을 실행하는 것이 효율적이다. 이런 작업을 위해 **복수의 코루틴의 실행이 모두 끝날 때까지 호출부의 코루틴을 일시
  중단시키는 joinAll 함수**가 있다.
...
- 이 일시 중지 기능은 취소할 수 있습니다. 이 일시 중지 기능이 대기하는 동안 현재 코루틴의 작업이 취소되거나 완료되면 이 기능은 즉시 취소 예외와 함께 재개됩니다. 신속한 취소 보장이 있습니다. 이 기능이 일시
  중지된 상태에서 작업이 취소된 경우 성공적으로 재개되지 않습니다. 자세한 내용은 **suspendCancellableCoroutine** 문서를 참조하십시오.
...
> 1025 [DefaultDispatcher-worker-1] 이미지1 변환 완료
> 1025 [DefaultDispatcher-worker-2] 이미지2 변환 완료
...
- 위 코드를 실행하면 아무 로그도 나오지 않는다. **지연 코루틴은 명시적으로 실행을 요청하지 않으면 실행되지 않기 때문**이다.
...
- 위 코드에서는 lazyJob 지연 코루틴이 만들어진 후 delay를 통해 1초간 대기 후 lazyJob.start()가 호출되어 지연 코루틴이 실행된다. 이처럼 **지연 코루틴은 생성 후 자동으로 실행되지 않고
  직접 실행을 호출해야 한다.**
...
- 이런 문제를 해결하기 위해 **Job 객체는 코루틴을 취소할 수 있는 cancel 함수를 제공**한다.
...
- 이 코드는 잘 동작할 것 같지만, **순차성 관점에서 중요한 문제점**이 있다. Job객체에 cancel을 호출하면 **코루틴은 즉시 취소되는 것이 아니라 Job 객체 내부의 취소 확인용 플래그를 ‘취소
  요청됨’으로 변경함으로써 코루틴이 취소되어야 한다는 것만 알린다.** 이후 미래의 어느 시점에 코루틴의 취소가 요청됐는지 체크하고 취소된다.
- 즉, cancel 함수를 사용하면, cancel의 대상이 된 job 객체는 곧바로 취소되는 것이 아니라 **미래의 어느 시점에 취소**된다. 이런 문제 때문에 이처럼 코드를 작성하면 longJob 코루틴이 취소된
...
- cancel 함수나 cancelAndJoin 함수를 사용했다고 해서 코루틴이 즉시 취소되는 것은 아니다. 이들은 Job 객체 내부에 있는 취소 확인용 플래그를 바꾸기만 하며, 코루틴은 이 플래그를 확인하는 시점에
  비로소 취소된다. 만약 **코루틴이 취소를 확인할 수 있는 시점이 없다면 취소는 일어나지 않는다.**
...
- whileJob 코루틴이 취소될 것 같지만, **취소되지 않는 이유는 코루틴 블록 내부에 코루틴의 취소를 확인할 수 있는 시점이 없기 때문이다.** 코루틴은 일반적으로 실행 대기 시점이나 일시 중단 지점에 취소를
...
- 작업 중이 출력될 때마다 yield로 인해 일시 중단이 일어난다. 이 코드에서 Dispatchers.Default를 사용하는 코루틴은 whileJob밖에 없으므로 whileJob 코루틴은 **yield를 호출하면
  일시 중단 후 곧바로 재개되지만 잠깐 일시 중단된 시점에 취소 체크가 일어난다.**
- 하지만 **yield를 사용하는 방법 또한 while문을 한 번 돌 때마다 스레드 사용이 양보되면서 일시 중단되는 문제**가 있다. 코루틴이 아무리 경량 스레드라고 하더라도 매번 일시 중단되는 것은 작업을
  비효율적으로 만든다.
...
- **이 방법을 사용하면 코루틴이 잠시 멈추지도 않고 스레드 사용을 양보하지도 않으면서 계속해서 작업을 할 수 있어서 효율적**이다. 코드를 실행해 보면 코루틴이 정상적으로 취소되는 것을 확인할 수 있다.
- 즉, **만약 코루틴 내부의 작업이 일시 중단 지점 없이 계속된다면 명시적으로 코루틴이 취소됐는지 확인하는 코드를 넣어줌으로써 코드를 취소할 수 있도록 만들어야 한다**. 그렇지 않으면 코루틴 취소가 동작하지
  않음을 명심하자.

## 4.6 코루틴의 상태와 Job의 상태 변수

![image.png](attachment:defcca75-b11d-4bfc-a46b-7131f2304dc5:image.png)

코루틴은 위와 같이 6가지의 상태를 가질 수 있다. (’실행 완료 중’은 7장에서 다룸)

- 생성
    - **코루틴 빌더를 통해 코루틴을 생성하면 코루틴은 기본적으로 생성 상태에 놓이며, 자동으로 실행 중 상태로 넘어간다**. 만약 생성 상태의 코루틴이 실행 중 상태로 자동으로 변경되지 않도록 만들고 싶다면
      코루틴 빌더의 start 인자로 CoroutineStart.Lazy를 넘겨 지연 코루틴을 만들면 된다.
- 실행 중
    - **지연 코루틴이 아닌 코루틴을 만들면 자동으로 실행 중 상태로 바뀐다**. 코루틴이 실제로 실행 중일 때뿐만 아니라 실행된 후에 일시 중단된 때도 실행 중 상태로 본다.
- 실행 완료
...
- **Job 객체는 코루틴이 어떤 상태에 있는지 나타내는 상태 변수들을 외부로 공개**한다. 다만, **Job 객체는 코루틴을 추상화 한 객체이므로 노출하는 상태 변수들은 코루틴의 상태를 간접적으로만 나타낸다.**
...
- isActive
    - 코루틴이 활성화되어 있는지의 여부. 코루틴이 활성화되어 있으면 true를 반환, 활성화되어 있지 않으면 false를 반환한다. **활성화되어 있다는 것은 코루틴이 실행된 후 취소가 요청되거나 실행이
      완료되지 않은 상태라는 의미**이다. 따라서, 취소가 요청되거나 실행이 완료된 코루틴은 활성화되지 않은 것으로 본다.
- isCancelled
    - 코루틴이 취소 요청되었는지의 여부. **코루틴이 취소 요청되면 true를 반환하는데 요청되기만 하면 true가 반환**되므로 isCancelled가 true이더라도 즉시 취소되는 것은 아니다.
- isCompleted
    - 코루틴이 실행 완료되었는지의 여부. **코루틴의 모든 코드가 실행완료되거나 취소 완료되면 true를 반환**하며, 실행 중인 상태에서는 false를 반환한다.
...
- 생성 상태는 코루틴이 생성만 되고 실행되지 않은 상태이다. 위의 사진처럼 지연 코루틴으로 코루틴을 생성한 경우이다. 코드 실행 결과를 보면, 코루틴이 생성된 후 실행되지 않았으므로 모두 다 false이다.
  ...
  ![스크린샷 2025-06-06 14.20.54.png](attachment:9af1eab5-4c67-4b19-9660-c72d496972c0:스크린샷_2025-06-06_14.20.54.png)
...
![coroutine_state2.jpg](attachment:e3542de4-bcc8-4491-a959-4116edd14960:coroutine_state2.jpg)
...
- 취소 요청된 코루틴은 활성화돼 있지 않다고 보고 isActive는 false가 되고, 취소가 요청되었으므로 isCancelled는 true가 되며, 취소가 완료되지 않았으므로 isCompleted는 false가
  된다.
- 여기서 중요한 점은 **취소가 요청되면 실제로는 코드가 실행 중이더라도 코루틴이 활성화된 상태로 보지 않는다는 점**이다.

### 4.6.6 취소 완료된 코루틴

- 코루틴은 **코루틴에 취소가 요청되고 취소 요청이 확인되는 시점(일시 중단 등)에 취소가 완료**된다.
...
![image.png](attachment:e9d56867-3321-4eb3-88af-452ef8030820:image.png)
